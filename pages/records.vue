  ...
  <NuxtLayout>
    ...
    >
      ...
      <div class="bg-inherit md:bg-white md:dark:bg-zinc-900 slideDown pt-8">
        ...
        <div
          v-for="(yearGroup, index) in groupedData"
          :key="index"
          class="px-6 md:px-12"
        >
          <div
            class="text-4xl md:text-5xl font-bold mt-10 mb-4 dark:text-zinc-300"
          >
            {{ yearGroup.year }}
          </div>
          <ul class="list-none p-0 m-0">
            <li
              ...
              :key="item.id"
              class="flex justify-between items-center hover:border-b dark:border-zinc-700 py-2 space-y-4"
            >
              <span class="text-left">
                <NuxtLink
                  :to="item._path"
                  noPrefetch
                  class="flex-none w-fit md:text-xl dark:text-zinc-400 hover:text-zinc-500 dark:hover:text-zinc-300"
                  >{{ item.title }}</NuxtLink
                >
              </span>
              <span class="text-right dark:text-zinc-400">{{
  ...
  meta: [
    {
      name: "keywords",
      content:
        "records,LYP.INK,Developer,Blogger,Blog,Personal Site,Nuxt.js,Vue.js,JavaScript,Tailwindcss",
    },
...
const articles = await queryContent("record")
  .only(["date", "last", "description", "title", "_path", "show"])
  .find();
...
if (isProd()) {
  data.value = articles.filter(
    (item) => "/records" !== item._path && false != item?.show
  );
} else {
