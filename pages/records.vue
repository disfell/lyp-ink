		...
		<div class="mx-auto max-w-2xl min-w-0">
			...
			<div class="md:backdrop-blur-sm md:bg-zinc-50/30 md:dark:bg-zinc-700/30 md:shadow-inner py-10">
				...
				<div v-for="(yearGroup, index) in groupedData" :key="index" class="px-6">
					<h2 class="text-2xl font-bold my-2 dark:bg-clip-text dark:text-transparent dark:bg-gradient-to-r dark:from-stone-100 dark:to-stone-500 article-item">{{ yearGroup.year }}</h2>
					<ul class="list-none p-0 m-0">
						<li v-for="item in yearGroup.items" :key="item.id" class="flex justify-between items-center border-dashed border-b dark:border-gray-600/50 py-2 article-item">
