  ...
  <Transition name="slide-fade">
    <div v-if="platform && currentApp" class="fixed top-4 right-0">
      <div ref="slidePanel" class="relative cursor-pointer">
        <div
          class="backdrop-blur-md border border-zinc-300 dark:border-zinc-700 rounded-l-lg bg-zinc-50 dark:text-zinc-100 dark:bg-zinc-800 p-4 shadow transition-transform duration-500 ease-in-out"
          :style="panelStyle"
          @click="togglePanel"
        >
...
onMounted(() => {
  if (conf.public.useRealtime === "true") {
    ...
    supaChannel
      .on("broadcast", { event: "mystatus" }, (payload) =>
...
watch(isPanelVisible, (newValue, oldValue) => {
  const transformValue = newValue
    ? "translateX(0)"
    : `translateX(${slidePanel.value.clientWidth - 20}px)`;
